...
# ── Pre-fetch images and build cowrie's custom image ──────────────────
# Pull happens here so network downloads overlap with later setup steps.
# Build is done explicitly (not via "up --build") to avoid concurrent
# builds with mysql-honeypot, which crashes BuildKit.
echo "[cowrie] Pre-fetching images..."
cd "${DEPLOY_DIR}"
docker compose pull

echo "[cowrie] Building analyzer image..."
docker compose build analyzer

echo "[cowrie] Cowrie configured — stack will start after all honeypots are set up."
